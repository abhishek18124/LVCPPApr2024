...
TreeNode* deleteFromBST(TreeNode* root, int key) {
	...
	if (key < root->val) {
		...
		// recursively, delete the key, if present, from the leftSubtree which is also a BST

		root->left = deleteFromBST(root->left, key);

	} else if (key > root->val) {

		// recursively, delete the key, if present, from the rightSubtree which is also a BST
