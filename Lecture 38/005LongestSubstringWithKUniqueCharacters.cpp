...
int main() {
	...
	// time : O(n)
	// space: O(n) due to freqMap assuming all characters in s are unique (max. 26)
	...
	while (j < n) {
		...
		if (uniqCnt > k) {
			// window property is violated therefore start shrinking the window
			while (uniqCnt > k) {
				// shrink the window
				freqMap[s[i]]--;
				if (freqMap[s[i]] == 0) {
		...
		if (uniqCnt == k) {
			// we've found a valid window that starts at the ith index and ends at the jth index
